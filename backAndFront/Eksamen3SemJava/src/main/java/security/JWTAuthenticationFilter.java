...
public class JWTAuthenticationFilter  implements ContainerRequestFilter {
    ...
    public void filter(ContainerRequestContext request) throws IOException {
        if (isSecuredResource()) {
            ...
            if (token == null) {
                request.abortWith(exception.EksamenException.makeErrRes("Missing token", token, 403));
                return;
            }
            try {
                ...
                request.abortWith(exception.EksamenException.makeErrRes("Invalid Token", "Tome out", 400));
            }
        }
    }
